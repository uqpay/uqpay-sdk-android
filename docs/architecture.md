...
- Outcome-unknown (`IOException`, `429`, `5xx`, un-parseable `2xx`) **keeps** the pin;
  a definitive answer clears it; cancellation leaves it untouched.
...
- Non-persistent cookies/storage, cleared on teardown.
...
- Theming through `UQPayAppearance` (colours, corner radius, merchant display name),
  with a real light/dark scheme. iOS force-disables dark mode in seven view controllers
  while its public API advertises adaptive colours (audit item U3); ours honours the
  system theme, with an explicit override for hosts on API < 29 where there is no
...
- All strings in `strings.xml` from the first commit (iOS has zero localization, audit
  item U4). English ships; more can be added without touching code.
...
`UQPayError` carries `code`, `message` (human-readable, always safe to log),
`declineCode: String?` (the acquirer's raw code) and an internal `cause`.
